...
from PySide6.QtGui import QDesktopServices, QFont, QStandardItemModel, QShowEvent, QFontDatabase, QIcon, QPalette
from PySide6.QtWidgets import QMainWindow, QApplication
from PySide6.QtWidgets import QFileDialog, QMessageBox
from PySide6.QtWidgets import QTreeView, QTextBrowser, QWidget
...
class MainWindow(QMainWindow):
    ...
    # Widget types that render text content (as opposed to controls like buttons/labels) and should
    # use a portable monospace font so their content stays visually aligned.
    MONOSPACE_WIDGET_TYPES = (QTreeView, QTextBrowser)
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.logger = logging.getLogger("dcmspec_explorer")
        ...
        self._load_details_css()
        self._select_icons()
        self._setup_explanation_drawer()
        self._connect_signals()

    def _setup_fonts(self) -> None:
        """Set the fonts for the main window's widgets."""
        # Set monospace font for all widgets defined as monospace type
        font = self.get_portable_monospace_font()
        for widget_type in self.MONOSPACE_WIDGET_TYPES:
            matches: list[QWidget] = self.findChildren(widget_type)
        ...
        header_font = QFont(default_font.family(), default_font.pointSize() - 1, QFont.Weight.Bold)
        header.setFont(header_font)

    def _setup_treeview(self) -> None:
        """Configure the treeview widget header, context menu, and styling."""
        header = self.ui.iodTreeView.header()
        ...
        self.ui.iodTreeView.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)

        # Only show a vertical bar between Name and Kind column headers
    ...
    def _select_icons(self) -> None:
        ...
        self.heart_icon = self._load_icon("heart.svg")
        self.cached_icon = self._load_icon("cached.svg")
        self.uncached_icon = self._load_icon("uncached.svg")

        is_dark = self.palette().color(QPalette.ColorRole.Window).lightness() < 128
        close_icon_name = "close-dark.svg" if is_dark else "close.svg"
        self.close_icon = self._load_icon(close_icon_name)

    def _setup_explanation_drawer(self) -> None:
        """Set up the collapsible explanatory section drawer, initially hidden."""
        if self.close_icon is not None:
            self.ui.explanationCloseButton.setIcon(self.close_icon)
